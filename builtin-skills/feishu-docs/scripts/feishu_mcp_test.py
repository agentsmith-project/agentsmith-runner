...
class FeishuMcpTests(unittest.TestCase):
    ...
    def test_explicit_access_token_does_not_bypass_projection(self, mock_resolve: MagicMock) -> None:
        ...
        args = argparse.Namespace(**{"access_" + "tok" + "en": "manual_arg"})
    ...
    def test_parser_rejects_explicit_endpoint_arg(self) -> None:
        parser = feishu_mcp.build_parser()

        with patch("sys.stderr", new_callable=StringIO) as stderr, self.assertRaises(SystemExit) as raised:
            parser.parse_args(["tools-list", "--mcp-endpoint", "https://mcp.feishu.cn/mcp"])

        self.assertEqual(raised.exception.code, 2)
        self.assertIn("unrecognized arguments: --mcp-endpoint", stderr.getvalue())
    ...
    def test_rpc_call_uses_projected_endpoint_and_token(
        ...
        mock_resolve.return_value = {
            "fields": {
                ...
                "endpoint": "https://mcp.feishu.cn/mcp/projected",
            }
        }
        response = MagicMock()
        ...
        self.assertEqual(req.full_url, "https://mcp.feishu.cn/mcp/projected")
        self.assertEqual(req.headers["X-lark-mcp-uat"], "token_projected")

    @patch("feishu_mcp.urlopen")
    @patch("feishu_mcp.resolve_projected_dependency")
    def test_env_endpoint_override_is_ignored(
        self,
        mock_resolve: MagicMock,
        mock_urlopen: MagicMock,
    ) -> None:
        mock_resolve.return_value = {"fields": {"access_token": "token_projected"}}
        response = MagicMock()
        response.read.return_value = json.dumps({"result": {"ok": True}}).encode("utf-8")
        mock_urlopen.return_value.__enter__.return_value = response
        args = argparse.Namespace()

        with patch.dict(os.environ, {"FEISHU_MCP_ENDPOINT": "https://evil.example.test/mcp"}):
            result = feishu_mcp.rpc_call(args, "tools/list", {}, "search-doc")

        self.assertEqual(result, {"result": {"ok": True}})
        req = mock_urlopen.call_args.args[0]
        self.assertEqual(req.full_url, feishu_mcp.ENDPOINT)

    @patch("feishu_mcp.urlopen")
    @patch("feishu_mcp.resolve_projected_dependency")
    def test_untrusted_projected_endpoint_fails_fast(
        self,
        mock_resolve: MagicMock,
        mock_urlopen: MagicMock,
    ) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "not trusted"):
            feishu_mcp.rpc_call(args, "tools/list", {}, "search-doc")

        mock_urlopen.assert_not_called()

    @patch("feishu_mcp.urlopen")
    @patch("feishu_mcp.resolve_projected_dependency")
    def test_non_https_projected_endpoint_fails_fast(
        self,
        mock_resolve: MagicMock,
        mock_urlopen: MagicMock,
    ) -> None:
        mock_resolve.return_value = {
            "fields": {
                "access_token": "token_projected",
                "endpoint": "http://mcp.feishu.cn/mcp",
            }
        }
        args = argparse.Namespace()
